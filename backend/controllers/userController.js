...
function determineGroup(age) {
  ...
  else if (age >= 6) return 'Enfence/Réseau'; // This one is consistent with the comment.
  else return null;
}
...
/**
 * NEW FUNCTION: Remove deviceId from a user's record (for logout).
...
exports.removeDeviceId = async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      userId,
      { $set: { deviceId: null } }, // Set deviceId to null
